...
async def steal_sticker(interaction: discord.Interaction, message: discord.Message):
    ...
    if not message.stickers:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=interaction.client.color,
                description=f"{interaction.client.no} {interaction.user.mention}: This message doesn't have any sticker",
            ),
            ephemeral=True,
        )
    try:
        ...
        embed = discord.Embed(
            color=interaction.client.color,
            description=f"{interaction.client.no} {interaction.user.mention}: unable to add this sticker - {error}",
        )
...
async def grab_sticker(interaction: discord.Interaction, message: discord.Message):
    ...
    if len(options) == 0:
        return await interaction.response.send_message(
            embed=discord.Embed(
                color=interaction.client.color,
                description=f"{interaction.client.no} {interaction.user.mention}: You can't add this sticker anywhere",
            ),
            ephemeral=True,
        )
    embed = discord.Embed(
        color=interaction.client.color,
        description=f"🔍 Where should `{message.stickers[0].name}` be addded?",
    )
    ...
    async def select_callback(inte: discord.Interaction):
        ...
        await inte.response.edit_message(
            embed=discord.Embed(
                color=interaction.client.color,
                description=f"{interaction.client.yes} {inte.user.mention}: Added `{sticker.name}` in **{len(select.values)}** servers",
            ),
            view=None,
        )
...
async def user_avatar(interaction: discord.Interaction, member: discord.Member):
    button1 = Button(
        label="default avatar", url=member.avatar.url or member.default_avatar.url
    )
...
async def user_banner(interaction: discord.Interaction, member: discord.Member):
    ...
    if not user.banner:
        if user.accent_colour is None:
            return await interaction.client.ext.send_error(
                interaction,
                "**{}** Doesn't have a banner".format(str(user)),
                ephemeral=True,
            )
        ...
        try:
            ...
            if check:
                ...
                e.set_author(
                    ...
                    url="https://discord.gg/4D5zzsYcUx",
...
class cog(commands.Cog):
    def __init__(self, bot: commands.AutoShardedBot):
        ...
        try:
            ...
            self.bot.tree.remove_command(
                "user avatar", type=discord.AppCommandType.message
            )
            self.bot.tree.remove_command(
            ...
            self.bot.tree.remove_command(
                "user banner", type=discord.AppCommandType.user
            )
            self.bot.tree.add_command(user_avatar)
            self.bot.tree.add_command(steal_sticker)
            self.bot.tree.add_command(grab_sticker)
    ...
    async def confess(self, ctx: discord.Interaction):
        ...
        if re:
            await ctx.client.ext.send_error(
                ctx,
