...
def get_progress(xp, level):
    corner_black_left = "<:blue_left_rounded:1121826878496456786>"
    black = "<:blue:1121826869671637094>"
    corner_black_right = "<:blue_right_rounded:1121826882321645648>"
    corner_white_left = "<:white_left_rounded:1131231568019595365>"
    white = "<:white:1131231515427221544>"
    corner_white_right = "<:white_right_rounded:1131231680028479650>"
...
class Leveling(commands.Cog):
    ...
    @commands.hybrid_command(
        aliases=["lvl", "checklevel"],
        help="check any member's rank",
        usage="[member], ",
    )
    ...
    @level.group(
        ...
        help="manage the rewards for each level",
    )
    async def rewards(self, ctx: commands.Context):
        await ctx.create_pages()

    @rewards.command(
        help="add a level reward",
        usage="[level] [role]",
        brief="manage guild",
    )
    ...
    @rewards.command(help="return a list of role rewards")
    ...
    @level.command(
        name="reset",
        help="reset levels for everyone",
        brief="server owner",
        usage="<member>",
    )
    @Perms.server_owner()
    ...
    @level.command(aliases=["lb"], help="check level leaderboard")
    async def leaderboard(self, ctx: commands.Context):
        ...
        for table in results:
            ...
            level = table["level"]
            num += 1
            auto += f"\n{'<a:lolzies:1109878310634278932>' if num == 1 else f'`{num}`'} **{await self.bot.fetch_user(user) or user}** - **{exp}** xp (level {level})"
            k += 1
            l += 1
            if l == 10:
                break
    ...
    @level.command(
        help="enable leveling system.. or disable it",
        brief="manage guild",
    )
    @Perms.get_perms("manage_guild")
    ...
    @level.command(
        help="set where the level up message should be sent",
    ...
    @level.command(
        help="set a channel to send level up messages",
        usage="[channel]",
        brief="manage guild",
    )
    @Perms.get_perms("manage_guild")
