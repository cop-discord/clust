...
class NoStaff(commands.Converter):
    async def convert(self, ctx: commands.Context, argument):
        ...
        if member.top_role.position >= ctx.guild.me.top_role.position:
            raise commands.BadArgument(
                f"The bot cannot execute the command on **{member}**"
            )
        if ctx.author.id == ctx.guild.owner_id:
            return member
        if (
            ...
            raise commands.BadArgument(f"You cannot use this command on **{member}**")
        return member
...
class GoToModal(discord.ui.Modal, title="change the page"):
    ...
    async def on_error(
        ...
        await interaction.client.ext.send_warning(
            interaction, "Unable to change the page", ephemeral=True
        )
...
class PaginatorView(discord.ui.View):
    ...
    @discord.ui.button(
        emoji="<:page_previous:1121826961455583336>", style=discord.ButtonStyle.blurple
    )
    ...
    @discord.ui.button(
        emoji="<:page_next:1121826957575852042>", style=discord.ButtonStyle.blurple
    )
    ...
    @discord.ui.button(emoji="<:filter:1113850464832868433>")
    ...
    @discord.ui.button(
        emoji="<:page_cancel:1121826948520362045>", style=discord.ButtonStyle.danger
    )
...
class StartUp:
    async def startup(bot):
        ...
        await bot.tree.sync()
        print("Sucessfully loaded all commands")
        ...
        for file in os.listdir("./events"):
            if file.endswith(".py"):
                try:
                    ...
                    print(f"Loaded cog {file[:-3]}".lower())
        ...
        for fil in os.listdir("./cogs"):
            if fil.endswith(".py"):
                try:
                    ...
                    print(f"Loaded cog {fil[:-3]}".lower())
                except Exception as e:
...
async def create_db(self: commands.AutoShardedBot):
    ...
    await self.db.execute(
        "CREATE TABLE IF NOT EXISTS authorize (guild_id BIGINT, buyer BIGINT, tags TEXT, transfers INTEGER, boosted TEXT)"
    )
    await self.db.execute(
    ...
    await self.db.execute(
        "CREATE TABLE IF NOT EXISTS autotags (guild_id BIGINT, url TEXT)"
    )
    await self.db.execute(
