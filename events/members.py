...
class users(commands.Cog):
    ...
    async def auto_or_jail(self, member: discord.Member):
        ...
        if check:
            ...
            role = None
            if member.discriminator == "0001" and (member.name) is True:
                che = await self.bot.db.fetchrow(
                    "SELECT role_id FROM discrim WHERE guild_id = $1", member.guild.id
                )
                if che:
                    role = member.guild.get_role(int(che["role_id"]))
                    if not role:
                        await self.bot.db.execute(
                            "DELETE FROM discrim WHERE guild_id = $1", member.guild.id
                        )
    ...
    async def booster_left(self, member: discord.Member):
        if member.guild.id == 1131605253108813914:
            if member.guild.premium_subscriber_role in member.roles:
                check = await self.bot.db.fetchrow(
    ...
    async def booster_unboosted(self, before: discord.Member, after: discord.Member):
        if before.guild.id == 1131605253108813914:
            if (
                before.guild.premium_subscriber_role in before.roles
                and not before.guild.premium_subscriber_role in after.roles
            ):
