import discord, json, datetime, typing, aiohttp, difflib, sys, io, os, humanize, importlib, inspect, math
...
async def help(ctx):
    ...
    embed.set_thumbnail(url="attachment://clust.png")
...
{cmd.brief}'''}```""",
    ...
    await ctx.reply(embed=embed, file=discord.File("clust.png"))
...
async def to_object(params):
    ...
    async for part in aiter(get_parts(params)):
        ...
        if part.startswith("color:"):
            try:
                x["color"] = int(
                    part[len("color:") :].replace(" ", "").replace("#", ""), 16
                )
            except:
                x["color"] = 0x2B2D31
        ...
        if part.startswith("attach:"):
            ...
            files.append(
                discord.File(
                    io.BytesIO(balls),
                    yarl.URL(part[len("attach:") :].replace(" ", "")).name,
                )
            )
...
async def send_help(ctx):
    ...
    if isinstance(ctx.command, commands.Command):
        ...
        embed.set_footer(
            ...
            icon_url="https://cdn.discordapp.com/attachments/1107734070659653652/1121870697719939152/Cherry_Blossoms.png",
        )
        embed.set_author(
        ...
        paginator.add_button("prev", emoji="<:page_previous:1121826961455583336>")
        paginator.add_button("goto", emoji="<:filter:1113850464832868433>")
        paginator.add_button("next", emoji="<:page_next:1121826957575852042>")
...
class MessageSnipe:
    def __init__(
        ...
        self._author_color = 0x2B2D31 if not author.color else author.color
    ...
    def embed(self) -> discord.Embed:
        embed = discord.Embed(color=self._author_color, description=self.content)
        embed.set_author(
            name=self.sender.display_name, icon_url=self.sender.display_avatar.url
        )
        embed.set_footer(
            text=f"deleted {self.timestamp} ago", icon_url=self.bot.footerIcon
        )
