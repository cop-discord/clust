...
class goto_modal(discord.ui.Modal, title="clust pagination"):
    def __init__(self, button):
        super().__init__()
        self.button = button
        self.page_num = discord.ui.TextInput(
            label="clust Cmd Menu",
        ...
        if not self.invoker:
            ...
            if interaction.user.id != self.invoker:
                return await interaction.response.send_message(
                    ...
                    embed=discord.Embed(
                        color=0x2B2D31,
                        description=f"<:warnings:1134940254424539277> {interaction.user.mention}: you can't **interact** with this",
                    ),
                )
            else:
...
def embed_creator(
    ...
    return [
        discord.Embed(
            ...
            color=color if color != None else colour if colour != None else 0x2B2D31,
        )
