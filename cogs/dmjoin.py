...
class Joindm(commands.Cog):
    ...
    @commands.group(
        ...
        help="manage dm's on member join",
    )
    ...
    @joindm.command(
        name="add",
        help="add a message that will be sent to any new member that will join the server",
        brief="manage guild",
    ...
    async def joindm_add(self, ctx: commands.Context, *, message: str):
        ...
        await ctx.send_success(f"Configured **joindm** message to\n```{message}```")

    @joindm.command(
        name="remove",
        help="remove the joindm message",
        brief="manage guild",
    )
    @Perms.get_perms("manage_guild")
    async def joindm_remove(self, ctx: commands.Context):
        ...
        if not check:
            return await ctx.send_warning("Joindm not enabled")
        await self.bot.db.execute(
            "DELETE FROM joindm WHERE guild_id = $1", ctx.guild.id
        )
        return await ctx.send_success("Deleted joindm")

    @joindm.command(
        name="test",
        help="test the joindm message",
        brief="manage guild",
    )
    @Perms.get_perms("manage_guild")
    async def joindm_test(self, ctx: commands.Context):
        ...
        if x[0] and x[1]:
            ...
            try:
                ...
                pass
        return await ctx.send_success("Check dm's if you received one")
