    ...
    async def create(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        if len(results) == 0:
            ...
            embed = discord.Embed(
                ...
                help="Support will be with you shortly\nTo close the ticket please press <:trash:1136338907101937845>",
            )
            embed.set_footer(
                text="/clustbot", icon_url=interaction.client.user.display_avatar.url
            )
            await interaction.client.db.execute(
        ...
        async def close_callback(inte: discord.Interaction):
            ...
            if check is not None:
                ...
                embed = discord.Embed(
                    color=inte.client.color,
                    title="ticket logs",
                    help="Logs for ticket `{}` | closed by **{}**".format(
...
class Tickets(commands.Cog):
    ...
    @ticket.command(
        help="add a person to the ticket",
        usage="[member]",
    ...
    @ticket.command(help="manage the ticket topics", brief="administrator")
    ...
    @ticket.command(
        help="configure the ticket message",
        usage="[embed code]",
        brief="administrator",
    )
    @has_admin_permissions()
    ...
    @ticket.command(
        help="configure the ticket category",
        usage="[category]",
        brief="administrator",
    )
    @utils.get_perms("administrator")
    ...
    @ticket.command(
        help="configure the ticket channel",
        usage="[channel]",
        brief="administrator",
    )
    @utils.get_perms("administrator")
    ...
    @ticket.command(
        help="configure the ticket logging channel",
        usage="[channel]",
        brief="administrator",
    )
    @utils.get_perms("administrator")
    ...
    @ticket.command(help="sends the ticket panel", brief="administrator")
    @utils.get_perms("administrator")
    async def send(self, ctx: commands.Context):
        ...
        if check["message"]:
            ...
            embed = discord.Embed(
                ...
                help="Click on the button below this message to create a ticket",
            )
            embed.set_author(
    ...
    @ticket.command(help="check the ticket panel's settings")
    async def settings(self, ctx: commands.Context):
        ...
        embed = discord.Embed(
            ...
            help="settings for **{}**".format(ctx.guild.name),
        )
        embed.add_field(
            name="ticket channel",
